...
## Mehrere OpenAI-Konten

OpenCode unterstützt die gleichzeitige Verwendung mehrerer OpenAI-Konten durch die Erstellung benutzerdefinierter Anbieter.

### Voraussetzungen

- OpenCode CLI oder TUI installiert
- Mehrere OpenAI-Konten (ChatGPT Plus/Pro oder separate API-Schlüssel)

### Einrichtung

#### 1. Benutzerdefinierter Anbieter konfigurieren

Fügen Sie mehrere OpenAI-basierte Anbieter zu Ihrer `opencode.json` hinzu:

```json title="opencode.json" {7-8}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai_work": {
...
- Die Anbieter-ID muss mit `openai` beginnen (z. B. `openai_work`, `openai_personlich`)
- Das Suffix darf Kleinbuchstaben, Zahlen, Bindestriche und Unterstriche enthalten
- Gültige Namen: `openai_work`, `openai_personlich`, `openai_2024`, `openai_team_a`

#### 2. Konten verbinden

Führen Sie `/connect` in der TUI oder CLI aus:

```bash
opencode auth login
```

Wählen Sie einen Ihrer benutzerdefinierten Anbieter:

- `OpenAI (Arbeit)` → `openai_work`
- `OpenAI (Persönlich)` → `openai_personlich`

Wählen Sie die Authentifizierungsmethode "ChatGPT Plus/Pro" und absolvieren Sie den OAuth-Flow.

#### 3. Mehrere Konten verwenden

Um zwischen Konten zu wechseln, führen Sie `/models` aus:

```bash
opencode models
```

Sie sehen alle Modelle von allen Ihren OpenAI-Konten:

- `openai_work/gpt-4o` - Modelle vom Arbeitskonto
- `openai_personlich/gpt-4o` - Modelle vom persönlichen Konto

Wählen Sie das Modell aus dem Konto, das Sie verwenden möchten.

### Authentifizierungsmethoden

#### ChatGPT Plus/Pro (OAuth)

Empfohlen für persönliche Konten mit ChatGPT-Abonnement:

- Automatische Token-Erneuerung
- Vollständiger Zugriff auf Codex-Modelle
- Keine API-Schlüsselverwaltung notwendig

#### API-Schlüssel

Für Team- oder Unternehmenskonten ohne ChatGPT Plus/Pro:

- API-Schlüssel erstellen unter https://platform.openai.com/api-keys
- Option "API-Schlüssel manuell eingeben" in `/connect` verwenden
- Manuelle Schlüsselrotation erforderlich

### Fehlerbehebung

#### Anbieter nicht in `/connect` angezeigt

- Überprüfen Sie das Format von `opencode.json`
- Stellen Sie sicher, dass die Anbieter-ID mit `openai` beginnt
- Überprüfen Sie auf Syntaxfehler in der Konfigurationsdatei

#### Authentifizierung fehlschlägt

- Vorhandene Anmeldeinformationen löschen: `opencode auth logout`
- Stellen Sie sicher, dass Sie das richtige Konto verwenden
- Überprüfen Sie die OAuth-Callback-URL (http://127.0.0.1:1455/callback)

#### Falsches Konto wird verwendet

- Überprüfen Sie, ob die Modellauswahl die Anbieter-ID enthält (z. B. `openai_work/gpt-4o`)
- Verwenden Sie `/models`, um alle verfügbaren Modelle anzuzeigen
- Führen Sie `opencode auth list` aus, um verbundene Konten zu überprüfen
